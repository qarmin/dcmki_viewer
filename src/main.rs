...
fn main() -> Result<(), AppError> {
    ...
    let search_state: Rc<RefCell<(String, bool)>> = Rc::new(RefCell::new((String::new(), false)));
    ...
    // File selected from directory list
    {
        ...
        window.on_file_selected(move |path: SharedString| {
            if let Some(w) = window_weak.upgrade() {
                ...
                *search_state.borrow_mut() = (String::new(), false);
            }
        });
    }
    ...
    // Open single file via dialog
    {
        ...
        window.on_open_file_dialog(move || {
                    ...
                    .pick_file()
            {
                ...
                load_and_apply(&path, &w, &tags_store, &entries_store, &frames_store, &collapsed);
                w.set_tag_search_text(SharedString::from(""));
                *search_state.borrow_mut() = (String::new(), false);
            }
        });
    ...
    // Search
    {
        ...
        let collapsed = collapsed_store.clone();
        let search_state = search_state.clone();
        window.on_search_changed(move |query: SharedString, advanced: bool| {
            if let Some(w) = window_weak.upgrade() {
                let q = query.to_lowercase();
                *search_state.borrow_mut() = (q.clone(), advanced);
                let all = tags_store.borrow();
                let filtered = rebuild_filtered(&all, &q, advanced, &collapsed.borrow());
    ...
    // Toggle sequence collapse
    {
        ...
        window.on_toggle_sequence(move |source_index: i32| {
            if let Some(w) = window_weak.upgrade() {
                ...
                let all = tags_store.borrow();
                let (q, advanced) = search_state.borrow().clone();
                let filtered = rebuild_filtered(&all, &q, advanced, &collapsed.borrow());
                w.set_filtered_tags(ModelRc::from(Rc::new(VecModel::from(filtered))));
            }
        });
    ...
    // Save current frame as PNG
    {
        ...
        window.on_save_current_frame(move || {
            if let Some(w) = window_weak.upgrade() {
                let idx = w.get_current_frame_index() as usize;
                let frames = frames_store.borrow();
                if let Some(img) = frames.get(idx)
                    && let Some(path) = rfd::FileDialog::new()
    ...
    // Save all frames as PNG files
    {
        ...
        window.on_save_all_frames(move || {
            ...
            if frames.len() == 1 {
                #[expect(clippy::indexing_slicing)] // Validated that 0 len is
                if let Some(path) = rfd::FileDialog::new()
                    .add_filter("PNG image", &["png"])
                    .set_file_name("frame.png")
                    .save_file()
                    && let Err(e) = frames[0].save(&path)
                {
                    error!("Save error: {e}");
                }
